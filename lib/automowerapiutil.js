...
module.exports = class AutomowerApiUtil {
  ...
  async login(force) {
    ...
    // If forced, or if the cache is invalid/expired, perform a new login
    if (force || !cachedLoginResponse || lastLogin <= Date.now() - (1000 * cachedLoginResponse.expires_in)) {
        loginPromise = this._login().then(response => {
            cachedLoginResponse = response;
            lastLogin = Date.now();
            loginPromise = null; // Reset the login promise once done
            return cachedLoginResponse; // Return the cached response after successful login
        }).catch(err => {
            loginPromise = null; // Reset the login promise in case of an error
            throw err;
        });

        // Wait for the ongoing login attempt to complete and return its result
        return loginPromise;
    }

    // If no new login is needed, return the cached login response
  ...
  async confirmCurrentError(mowerId) {
    let appkey = this.homey.settings.get('appkey');
    let token = await this.login();
    ...
    let that = this;
    const res = await fetch(`${baseAutomowerApiUrl}/mowers/${mowerId}/errors/confirm`, requestOptions)
      .catch(err => {
        this.homey.error(err);
        throw new Error(err);
      })
